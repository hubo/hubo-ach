...
int saveHomingParams( const char *file_name, int type )
{
    ...
    return printHomingParams( ptr_file, type, 0 );
}
...
int saveAllParams( const char *file_name )
{
    ...
    return printHomingParams( ptr_file, 0, 1 );
}
...
int printHomingParams( FILE* ptr_file, int type, int printAll )
{
    ...
    ach_channel_t cmd_chan;
    r = ach_open(&cmd_chan, HUBO_CHAN_BOARD_CMD_NAME, NULL);
    if( ACH_OK != r )
    {
        fprintf(stderr, "Failed to open %s!", HUBO_CHAN_BOARD_CMD_NAME);
        return -1;
    ...
    if(printAll)
    {
        fprintf(ptr_file, "# This table was automatically generated by the saveAllParmas( ~ ) function\n"
                          "# provided by libhuoparams. The values in this table are based on the board\n"
                          "# parameters on the hardware at the time that the function was run.\n\n"
                          "# WARNING: CANRate, HomeA_max and JamTime are not correct. It is unclear whether or not\n"
                          "#          Hubo-i or the CAN Documentation is incorrect.\n");
    }
    else
    {
    ...
    if(printAll)
    {
        fprintf(ptr_file, "Joint Kp   Ki   Kd   "
                                   "DeadZone  "
                                   "Offset      HomeLimit   LimDir  "
                                   "HomeV_max1  HomeV_max2  HomeA_max  LimitMode  "
                                   "EncRes   AutoScale  MotorDir  "
                                   "LowerLimit  UpperLimit  "
                                   "V_max  A_max  "
                                   "JamTime  JamDuty  PwrSatTime  "
                                   "InputErr  BigErr  TempWarn  "
                                   "CANRate  NumChans\n");
    }
    else
    ...
    for(i=0; i<HUBO_JOINT_COUNT; i++)
    {
        if(state.joint[i].active == 1 && params.joint[i].confidence == 1)
        {
            ...
            if( type==0 && printAll) // Raw and Save all params
            {
                fprintf(ptr_file,
    //                      The minus sign pads on the right instead of left
                           ...
                           "%-5d%-5d%-5d"           // Kp Ki Kd
                           "%-10d"                  // DeadZone
                           "%-12d%-12d%-8d"         // Offset, HomeLimit, LimDir
                           "%-12d%-12d%-11.3f%-11d"   // HomeV_max1, HomeV_max2, HomeA_max, LimitMode
                           "%-9d%-11d%-10d"         // EncRes, AutoScale, MotorDir
                           "%-12d%-12d"             // LowerLimit, UpperLimit
                           "%-7d%-7d"               // V_max, A_max
                           "%-9.2f%-9d%-12.2f"          // JamTime, JamDuty, PowerSatTime
                           "%-10d%-8d%-10d"         // InputErr, BigErr, TempWarn
                           "%-9d%-9d\n",            // CANRate, NumChannels
                            jointNames[i],
                            params.joint[i].Kp,
                            params.joint[i].Ki,
                            params.joint[i].Kd,
                            params.joint[i].deadZone,
                            params.joint[i].homeOffsetRaw,
                            params.joint[i].searchLimit,
                            params.joint[i].searchDirection,
                            params.joint[i].maxHomeLimitVelRaw,
                            params.joint[i].maxHomeOffsetVelRaw,
                            params.joint[i].maxHomeAccel,
                            params.joint[i].searchMode,
                            params.joint[i].encoderResolution,
                            params.joint[i].autoScale,
                            params.joint[i].motorDirection,
                            params.joint[i].lowerLimitRaw,
                            params.joint[i].upperLimitRaw,
                            params.joint[i].maxVelRaw,
                            params.joint[i].maxAccelRaw,
                            params.joint[i].jamTime,
                            params.joint[i].pwmDutyJam,
                            params.joint[i].pwmSaturationTime,
                            params.joint[i].maxInputDifference,
                            params.joint[i].maxError,
                            params.joint[i].maxError,
                            params.joint[i].canRate,
                            params.joint[i].boardType);
            }

            else if( type==0 && printAll==0) // Raw
            {
                fprintf(ptr_file,
    //                      The minus sign pads on the right instead of left
                           "%s         "        // Joint Name
