/*
 * can_ioctl - advcan CAN driver module
 *
...
int can_ioctl( __LDDK_IOCTL_PARAM )
{
   ...
   case CAN_IOCTL_COMMAND:
		...
		if  (ret < 0)
      {
         return -EINVAL;
	  	...
	  	((Command_par_t *) argp)->retval = can_Command(inode, (Command_par_t *)argp);
	  	((Command_par_t *) argp)->error = Can_errno;
	  	ret = __lddk_copy_to_user( 
               (Command_par_t *)arg, 
               (void *)argp,
	  				sizeof(Command_par_t));
		if  (  ret < 0  )
   ...
   case CAN_IOCTL_SEND:
	  	...
	  	if( !access_ok(VERIFY_WRITE, (void *)arg, sizeof(Send_par_t))) 
      {
	  	...
	  	ret = __lddk_copy_from_user( (void *)argp, 
               (Send_par_t *)arg,
	  				sizeof(Send_par_t));
		if ( ret < 0)
      {
         return -EINVAL;
	  	...
	  	((Send_par_t *) argp)->retval = can_Send(inode, ((Send_par_t *)argp)->Tx );
	  	((Send_par_t *) argp)->error = Can_errno;
	  	ret = __lddk_copy_to_user( (Send_par_t *)arg, 
               (void *)argp,
	  				sizeof(Send_par_t));
		if ( ret < 0)
      {
   ...
   case CAN_IOCTL_RECEIVE:
		...
		if ( ret < 0)
      {
         return -EINVAL;
	  	...
	  	ret = __lddk_copy_to_user( (Receive_par_t *)arg, 
               (void *)argp,
	  				sizeof(Receive_par_t));
		if(ret < 0)
      {
   ...
   case CAN_IOCTL_STATUS:
	  	if( !access_ok(VERIFY_READ, (void *)arg,sizeof(CanStatusPar_t))) 
      {
	     	DBGout(); 
			return(retval); 
	  	}
	  	if( !access_ok(VERIFY_WRITE, (void *)arg,sizeof(CanStatusPar_t))) 
      {
	     	DBGout(); 
			return(retval); 
	  	}
	  	...
	  	ret = __lddk_copy_to_user( (CanStatusPar_t *)arg, (void *)argp,
   ...
   case CAN_IOCTL_CONFIGURERTR:
	  	if( !access_ok(VERIFY_READ, (void *)arg,sizeof(ConfigureRTR_par_t)))
      {
	     	DBGout(); 
			return(retval); 
	  	}
	  	if( !access_ok(VERIFY_WRITE, (void *)arg,sizeof(ConfigureRTR_par_t)))
      {
	     	DBGout(); 
			return(retval); 
	  	}
	  	argp = (void *)kmalloc( sizeof(ConfigureRTR_par_t) +1 ,GFP_KERNEL );
	  	ret = __lddk_copy_from_user( (void *)argp, (ConfigureRTR_par_t *) arg,
		...
		if(ret < 0)
      {
         return -EINVAL;
	  	...
	  	ret = __lddk_copy_to_user( (ConfigureRTR_par_t *)arg, 
               (void *)argp,
	  			   sizeof(ConfigureRTR_par_t));
		if(ret < 0)
      {
...
int can_Send(struct inode *inode, canmsg_t *Tx)
{
   ...
   ret = __lddk_copy_from_user((canmsg_t *)&tx, (canmsg_t *)Tx, sizeof(canmsg_t));
	...
	)
{
   ...
   if( len > 0 )
   {
      ...
      ret = __lddk_copy_to_user((canmsg_t *) Rx, 
               (canmsg_t *) &rx,
               sizeof(canmsg_t));
      return rx.length;
   } 
