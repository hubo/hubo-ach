...
static _INLINE_ void transmit_chars(struct uart_8250_port *up)
{
	...
	int count = 0;
	unsigned int flags;	
	unsigned char save_tfl, save_ttl;
...
#endif
{
	...
	unsigned int dll, dlm, dlldlm, cpr, cpr73, cpr20, tcr;
	unsigned int save_mcr, save_lcr;
...
///lipeng
	...
	if(up->port.unused[1] & 0x01){
		...
		serial_icr_write(up, UART_CPR, (cpr73 << 3)|cpr20); /* CPR[7:3]=M */ 
	}
	else{
		serial_outp(up, UART_LCR, up->lcr | UART_LCR_DLAB);	/* set DLAB */
